...
use sentra_lib::risks::{
    RiskScanner, RuleDirectoryConfig, RuleLoadSummary, RuleType, ScanOptions, ensure_bundled_rules,
};
...
pub(crate) fn build_scan_options_with_cache(
    ...
    if options.rules.is_none() {
        options.rules = Some(ensure_bundled_rules(home)?);
    }
